...
def process_fix_def_naive(df, the_cuttof, a_cuttof, conf_cuttof, a_val=10, an_val=10, a_the_val=10):
    ...
    def fix(row):
        ...
        the_ratio = (100+row[def_ngram_count])/(100+row[indef_ngram_count])
        a_ratio = (100+row[indef_ngram_count])/(100+row[def_ngram_count])
    ...
    for n in ['r', 'w']:
        ...
        f=pd.DataFrame(m)
        f['trans'] = f['w']+' -> ' + f['cand']
        f=f[['cor', 'trans','suffix', 'score','the_count', 'a_count']]
...
def create_submission_arr(df, sents):
    ...
    def do_work(row):
        if row[correction] is not None:
            ...
            pos = row[position]
            arr[sent_index][pos] = (corr, conf, def_c, infef_c, row[suffix])
